...
public class MainActivity extends AppCompatActivity {
    protected static ActivityStatuses status = ActivityStatuses.RUNNING;
    ...
    private void updateTime_UI_THREAD() {
        Log.d("Threads","Running new thread");
        Thread thread=new Thread(new Runnable() {
            public void run() {
                try {
                    while(status!=ActivityStatuses.PAUSED){
    ...
    private void updateSettings() {
        ...
        switch (OPTION_TIME_FORMAT){
            case MENU_ITEM_24H: TIME_FORMAT="HH:mm:ss"; break;
            case MENU_ITEM_12H: TIME_FORMAT="hh:mm:ss aa"; break;
        }
    ...
    protected void onPause() {
        super.onPause();
        savePreferences();
        status=ActivityStatuses.PAUSED;
    }
    @Override
